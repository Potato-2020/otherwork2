    ...
    public static void install(@Nullable final Context context) {
        try {
            if (context == null) {
                ...
                if (oldHandler != null && oldHandler.getClass().getName().startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
                    ...
                    Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                        ...
                        public void uncaughtException(Thread thread, final Throwable throwable) {
                            if (config.isEnabled()) {
                                KLog.e("App has crashed, executing CustomActivityOnCrash's UncaughtExceptionHandler", throwable);

                                if (hasCrashedInTheLastSeconds(application)) {
                                    KLog.e("App already crashed recently, not starting custom error activity because we could enter a restart loop. Are you sure that your app does not crash directly on init?", throwable);
                                    if (oldHandler != null) {
                                    ...
                                    if (isStackTraceLikelyConflictive(throwable, errorActivityClass)) {
                                        KLog.e("Your application class or your error activity have crashed, the custom activity will not be launched!");
                                        if (oldHandler != null) {
                                            oldHandler.uncaughtException(thread, throwable);
                                            return;
                                        }
                ...
                KLog.i( "CustomActivityOnCrash has been installed.");
            }
        } catch (Throwable t) {
            KLog.e("An unknown error occurred while installing CustomActivityOnCrash, it may not have been properly initialized. Please report this as a bug if needed.", t);
        }
    }
    ...
    private static Class<? extends Activity> getRestartActivityClassWithIntentFilter(@NonNull Context context) {
        ...
        if (resolveInfos != null && resolveInfos.size() > 0) {
            ...
            try {
                ...
                KLog.e( "Failed when resolving the restart activity class via intent filter, stack trace follows!", e);
            }
        }
    ...
    private static Class<? extends Activity> getLauncherActivity(@NonNull Context context) {
        ...
        if (intent != null) {
            try {
                ...
                //Should not happen, print it to the log!
                KLog.e("Failed when resolving the restart activity class via getLaunchIntentForPackage, stack trace follows!", e);
            }
        }
    ...
    private static Class<? extends Activity> getErrorActivityClassWithIntentFilter(@NonNull Context context) {
        ...
        if (resolveInfos != null && resolveInfos.size() > 0) {
            ...
            try {
                ...
                //Should not happen, print it to the log!
                KLog.e( "Failed when resolving the error activity class via intent filter, stack trace follows!", e);
            }
        }
